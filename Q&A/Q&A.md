...
> 按主题分为五类：**控制基础**（Q1-Q2）、**变换器拓扑**（Q3-Q5）、**模型与代码**（Q6、Q8、Q11-Q12）、**算法改进**（Q7、Q9、Q13）、**仿真与工具**（Q10、Q14-Q15）。

## 问题索引

| 问题编号 | 分类 | 问题表述 |
|---------|------|---------|
| 1 | 控制基础 | MPPT控制频率对光伏电压是否有直接影响？ |
| 2 | 控制基础 | 在局部遮阴条件下，MPPT如何应对功率-电压特性中的多峰值问题？ |
| 3 | 变换器拓扑 | 在Buck-Boost转换器中，如何实现平滑模式过渡？TI的参考做法具体是什么？ |
| 4 | 变换器拓扑 | Buck-Boost转换器的输入电压波动问题是否可以通过稳压器解决？ |
| 5 | 变换器拓扑 | 能否设计一种AI观测模型来实时调整Buck-Boost的模式切换点？ |
| 6 | 模型与代码 | 肖克莱方程在MPPT模型中有不同形式（如是否包含串联电阻），差异是否源于对实际参数的细化？如何选择适合的方程版本？ |
| 7 | 算法改进 | 在MPPT算法代码中，添加环境突变检测（如光生电流突变）是否必要？现阶段实现是否可行？ |
| 8 | 模型与代码 | 在MPPT算法中，"光伏系统模型"具体指什么？是否需编写包含光照、温度等参数的详细模型？ |
| 9 | 算法改进 | 在扰动观察法（P&O）中，步长自适应机制（如delta = delta0 * abs(dP/dV)）是否可能导致步长过大、跳过最值点或振荡？如何改进（如使用非线性函数或震荡检测）？ |
| 10 | 仿真与工具 | 在Simulink中，MPPT控制电路除占空比输出外，能否用输出电压直接控制？如何修改代码实现？ |
| 11 | 模型与代码 | 在光伏代码模型中，通过调整参数绘制图像是否合理？图像对参数敏感是否正常？ |
| 12 | 模型与代码 | 将光伏模型和MPPT算法集成在一个代码文件中是否可行？还是应分离为函数调用？ |
| 13 | 算法改进 | 在代码中，使用查表法（预计算数据点）寻找最大功率点是否违背MPPT实时搜索原则？是否应改用连续算法？ |
| 14 | 仿真与工具 | 在Simulink仿真中，如何设置光伏模型参数和算法范围（如电压限制）以处理非线性特性？ |
| 15 | 仿真与工具 | 对于MPPT算法实现，Matlab代码能力要求是什么？是否需能编写完整逻辑程序处理数据？仿真主要用Simulink还是Multisim？ |
...
> 仓库对应：全局扫描+精跟的可运行实现见 `代码/global_scan_pv.m` 与 `python/global_scan_pv.py`；局部遮阴多峰现象分析见 `01-光伏发电基础/1.3 特殊工况分析：局部遮阴及其多峰值现象成因.md`。

### **3. 在Buck-Boost转换器中，如何实现平滑模式过渡？TI的参考做法具体是什么？**
...
### **4. Buck-Boost转换器的输入电压波动问题是否可以通过稳压器解决？**

理论上可以，但通常是**画蛇添足**，不推荐。

MPPT的本质就是要**让光伏板输出它所能提供的最大功率**，这个功率是随着光照温度变化的。因此，光伏板的输出电压（也就是Buck-Boost的输入电压）本身就应该是**变化的**。

* 如果你在它前面加一个稳压器，强行把电压稳定在一个值，那就等于把光伏板“掐住”了，它无法工作在自己特性的最优点上，**MPPT功能就失效了**。
* 输入电压的波动，应该通过优化Buck-Boost电路本身的控制环路（比如增加输入电压前馈补偿）来抑制，而不是在外面再加一个稳压器。

### **5. 能否设计一种AI观测模型来实时调整Buck-Boost的模式切换点？**
...
> 本节问题与仓库统一 PV 模型（`代码/pv_params.m`、`python/pv_model.py` 的单二极管模型）直接相关。
